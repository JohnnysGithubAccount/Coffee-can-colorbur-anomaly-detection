    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        with device_context:
            ...
            if save_segmentation_images:
                ...
                def image_transform(image):
                    # Check if transform_std and transform_mean exist
                    dataset = dataloaders["testing"].dataset
                    if hasattr(dataset, 'transform_std'):
                        in_std = np.array(dataset.transform_std).reshape(-1, 1, 1)
                    else:
                        in_std = np.ones((3, 1, 1))  # Default std for no transformation

                    if hasattr(dataset, 'transform_mean'):
                        in_mean = np.array(dataset.transform_mean).reshape(-1, 1, 1)
                    else:
                        in_mean = np.zeros((3, 1, 1))  # Default mean for no transformation
